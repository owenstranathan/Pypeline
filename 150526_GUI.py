...
'''FOR USE IN INFO PANEL'''
class NodeSingleChoicePanel(wx.Panel):
    def __init__(self, parent):

        wx.Panel.__init__(self, parent)

        choices = [u"Demand",u"Source", u"Wellhead", u"Junction", u"Control"]
...
class DenemeListPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.list_ctrl.InsertColumn(1, "TITLE", width=105)
        ...
        sizer.Add(self.list_ctrl, 0, wx.ALL|wx.FIXED_MINSIZE|wx.CENTRE|wx.TOP, 5)
        self.SetSizer(sizer)

'''FOR USE IN INFO PANEL SECOND ROW'''
class EditableListCtrl2(wx.ListCtrl, listmix.TextEditMixin):
    ''' TextEditMixin allows any column to be edited. '''
...
class DenemeListPanel2(wx.Panel):

    def __init__(self, parent):
        """Constructor"""
        wx.Panel.__init__(self, parent)

        rows = [("Source Node", "PP1 PP2 ")
                ]
        self.list_ctrl = EditableListCtrl2(self, style=wx.LC_REPORT)

        self.list_ctrl.InsertColumn(0, "NODE TYPE", width=115)
        self.list_ctrl.InsertColumn(1, "LINKS", width=115)
...
class InfoPanel(wx.Panel):
    def __init__(self, parent):
        ...
        sizer.Add(DenemeListPanel2(self), 0, wx.ALL|wx.EXPAND, 0)
        sizer.Add(NodeSingleChoicePanel(self), wx.ALL|wx.EXPAND, 0)
...
class PhysicalPanel(wx.Panel):
    def __init__(self, parent):
        ...
        box1_title = wx.StaticBox( panel, -1, "aaa" )
        ...
        sizerphysical.Add(sizerphysical1, 1, wx.ALL|wx.EXPAND, 5)
        sizerphysical.Add(sizerphysical2, 1, wx.ALL, 5)
...
class MethodRadioPanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.group1_ctrls = []
        radio1 = wx.RadioButton( panel, -1, "Not Defined", style = wx.RB_GROUP )
...
class SpinPanel(wx.Panel):
    def __init__(self, parent):
        ...
        spin = wx.SpinCtrlDouble(self, value='0.95', pos=(75,50), size=(54,-1),style=wx.SP_ARROW_KEYS,
                                 min=0.01, max=0.99, inc=0.01)
        spin.SetDigits(2)
...
class PropertiesPanel(wx.Panel):
    def __init__(self, parent):
        ...
        wx.Panel.__init__(self, parent, -1,size=(240,200))

        sizer = wx.BoxSizer(wx.VERTICAL)

        sizer.Add(MethodRadioPanel(self), 1, wx.ALL|wx.EXPAND, 0)
        sizer.Add(SpinPanel(self), 1, wx.ALL|wx.EXPAND, 0)
        self.SetSizer(sizer)
        sizer.Fit(self)
...
class NodeTabPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        sizer.Add(ListPanel(self, None, data=nodedata), 1, wx.ALL|wx.EXPAND, 5)

        sizer.Add(sizer2, 3, wx.ALL, 5)

        sizer2.Add(InfoPanel(self), 1, wx.ALL | wx.EXPAND | wx.FIXED_MINSIZE, 5)
        sizer2.Add(PhysicalPanel(self), 1, wx.ALL, 1)
        sizer2.Add(PropertiesPanel(self), 1, wx.ALL, 5)
