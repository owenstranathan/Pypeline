...
class SmoothArrowLine(PointsObjectMixin, LineOnlyMixin, DrawObject):
    """
    ArrowLine class definition.
...
class DotGrid:
    ...
    """
    def __init__(self, Spacing, Canvas, Size = 2, Color = "Black", Cross=False, CrossThickness = 1):

        self.Spacing = N.array(Spacing, N.float)
        self.Spacing.shape = (2,)
        self.Canvas = Canvas
        self.Size = Size / Canvas.Scale
        self.Color = Color
        self.Cross = Cross
...
class FloatCanvas(wx.Panel):
    ...
    def __init__(self, parent, id = -1,
        ...
        wx.EVT_LEFT_UP(self, self.LeftDownEvent)
        wx.EVT_LEFT_DCLICK(self, self.LeftDownEvent)
        wx.EVT_MIDDLE_DOWN(self, self.LeftDownEvent)
        wx.EVT_MIDDLE_UP(self, self.LeftDownEvent)
        wx.EVT_MIDDLE_DCLICK(self, self.LeftDownEvent)
        wx.EVT_RIGHT_DOWN(self, self.LeftDownEvent)
        wx.EVT_RIGHT_UP(self, self.LeftDownEvent)
        wx.EVT_RIGHT_DCLICK(self, self.LeftDownEvent)
        wx.EVT_MOTION(self, self.LeftDownEvent)
        wx.EVT_MOUSEWHEEL(self, self.LeftDownEvent)
        wx.EVT_KEY_DOWN(self, self.LeftDownEvent)
        wx.EVT_KEY_UP(self, self.LeftDownEvent)
...
def _makeFloatCanvasAddMethods(): ## lrk's code for doing this in module __init__
    classnames = ["Circle", "Ellipse", "Arc", "Rectangle", "ScaledText", "Polygon",
                  "Line", "Text", "PointSet","Point", "Arrow", "ArrowLine", "SmoothArrowLine", "ScaledTextBox",
